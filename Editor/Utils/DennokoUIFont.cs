...
namespace DenEmo.UI
{
    ...
    internal static class DennokoUIFont
    {
        ...
        // DenEmo の日本語 UI 文字列 (Utils/DenEmoLocalization.cs の JA 辞書) に出現する文字。
        // 文言を追加したら、ここにも新出文字を書き足すこと。
        private const string WarmupJapanese =
            "・ァアあィイいウうェエえォオおカかがキきクくグケけげコこごサさシしジじスすズず" +
            "セせゼそタたダだチッっつテてデでトとドどナなニにネのハはバばパヒビピフブプヘへ" +
            "ベべペホほボポマまみムむメめモもャヤやュョよラらリりルるレれロワわをンん" +
            "一上下不中乗了付以件体作使例保個値停側備元先入全内再処出分切初別制刻削前割加効" +
            "動化単原参反取可右各合同含因囲図在基報場変外大失奨始存完定実対導小尾崩左差常度" +
            "当後得応性情意態成戻所手択押括持指接推揃換操改敗新既日映時更書替最有期未末本条" +
            "棄検機次止正残気活消済満準点無照版状独現理生用画異登目直短破確示秒移窓競箇範系索" +
            "終結絞維綻緒編縮繋能自致色行表補製複見規覧解計記設認語読象起込追通速進適選部配" +
            "重録長閉開間防限除集非頂頭ー";
