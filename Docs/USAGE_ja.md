...
* **表示中をすべてチェック**: 現在リストに表示されている（検索やフィルターにマッチしている）シェイプキーをすべてチェックします（VRCAvatarDescriptor に登録されたまばたき・LipSync 用のシェイプは除外されます）。
...
- **トラック上のダイヤモンド** — 1 つのダイヤモンドが 1 つのキーフレームを表す。左右にドラッグすると別のフレームへ移動できる。重ねると `F:N  V:X.X` 形式のツールチップがキーの隣に表示される。右クリックでコンテキストメニューが開く：
  ...
  | メニュー項目 | 内容 |
  |------------|------|
  | 現在フレームにキーを一括追加 (N 個) | 現在リストに表示されている N 個のシェイプキーの現在値を、現在の再生ヘッド位置にキーとして一括追加する。REC モードのオン/オフに関わらず使用でき、検索・フィルターが有効な場合は表示中のシェイプキーのみが対象（N はその件数） |
  | 現在フレームの全キーを削除 | 現在の再生ヘッド位置にある全トラックのキーを一括削除する（確認ダイアログあり）。その位置にキーがなければグレーアウト |
  | 削除 | そのキーフレームを削除する |
...
- **タイムラインの空き領域を右クリック** — ルーラー・スクラバー・フレーム移動ハンドル行・トラックの空き領域（ダイヤモンド以外）のいずれを右クリックしても、「現在フレームにキーを一括追加」「現在フレームの全キーを削除」「現在時刻にペースト」のメニューが表示される。トラックが 1 つもない状態でも、ルーラーやスクラバーを右クリックすれば一括追加を実行できる

ラベル列の幅は、ラベルとトラックの境目の縦線をドラッグして変更できます。

**フレーム移動ハンドル行（トラック下部）**
すべてのトラックの下に、キーが存在する各フレームの位置へ `↔` ドラッグハンドルが並ぶ行が表示されます。ハンドルを左右にドラッグすると、そのフレームにある全トラックのキーをまとめて別のフレームへ移動できます。移動先に別のキーがある場合はその手前で停止し、ブロック元のキーが一瞬ハイライトされます。

> 例：REC をオフにしたまま特定フレームに全シェイプの現在ポーズを記録したいときは、スクラバーをそのフレームに合わせ、タイムラインを右クリック →「現在フレームにキーを一括追加」を選ぶだけで完了します。Ctrl+Z でまとめて 1 ステップ元に戻せます。

#### キーボードショートカット

タイムラインが表示されていてクリップが読み込まれており、テキストフィールド（フレーム番号入力欄・検索欄など）にフォーカスがない場合に、DenEmo ウィンドウ全体で有効です（タイムラインを別ウィンドウにしている場合はそのウィンドウで有効）。

シェイプキーのスライダーにフォーカスがある間は、`←` / `→` だけはスライダーの値調整に譲ります。それ以外のキーはそのまま効きます。
...
| **右クリックメニュー** | ダイヤモンドを右クリック →「フレームをコピー」（クリックしたフレームの全キーをコピー） | ダイヤモンド または タイムラインの空き領域を右クリック →「現在時刻にペースト」 |
...
> 例：フレーム 10 のダイヤモンドを右クリックして「フレームをコピー」し、スクラバーをフレーム 25 に移動してからタイムラインの空き領域を右クリック →「現在時刻にペースト」でも同じ複製ができます。
